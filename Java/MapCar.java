...
public class MapCar {
	...
	public static void main(String[] args) {
		...
		for (Entry<String, List<Map<String, String>>> entry : carByManufacturer.entrySet()) { // foreach前面的map
			String key = entry.getKey();
			List<Map<String, String>> list = carByManufacturer.get(key);
			minPriceSum = BigDecimal.ZERO;
			priceSum = BigDecimal.ZERO;
			for (int i = 0; i < list.size(); i++) {
				Map<String, String> mapDateil = list.get(i);
				System.out.printf("%-15s %-15s %10s %10s\n", entry.getKey(), mapDateil.get("Type"),
						list.get(i).get("Min.Price"), mapDateil.get("Price"));
				minPriceSum = minPriceSum.add(new BigDecimal(mapDateil.get("Min.Price")));
				priceSum = priceSum.add(new BigDecimal(mapDateil.get("Price")));
			}
